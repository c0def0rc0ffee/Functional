...
Each PNG has:
  ...
  * an alpha=255 white glyph (cog / 2x2 grid / power / star / lines / bars) on top
...
def base_disc():
    """
    <summary>
    Start a new icon: a transparent canvas with the semi-transparent white disc.
    </summary>
...
def cog(path: Path):
    """
    <summary>
    Eight tooth cog with the centre punched back to the disc, for the Settings button.
    </summary>
...
def puzzle(path: Path):
    """
    <summary>
    2x2 grid of rounded squares, for the Add-ons button.
    </summary>
    <param name="path">Where the PNG is written.</param>
    """
    img, d = base_disc()
...
def power(path: Path):
    """
    <summary>
    Power symbol: an open ring with a vertical bar through the gap at the top, for the Power button.
    </summary>
    <param name="path">Where the PNG is written.</param>
    """
    img, d = base_disc()
...
def star(path: Path):
    """
    <summary>
    Five point star with the first point straight up, for the Favourites button.
    </summary>
    <param name="path">Where the PNG is written.</param>
    """
    img, d = base_disc()
...
def bars(path: Path):
    """
    <summary>
    Three rising vertical bars, for the Stats corner button.
    </summary>
    <param name="path">Where the PNG is written.</param>
    """
    img, d = base_disc()
    ...
    base = SIZE / 2 + 31
    for h in heights:
        d.rounded_rectangle([x, base - h, x + bar_w, base], radius=4, fill=GLYPH_COLOUR)
        x += bar_w + gap
    img.save(path)
