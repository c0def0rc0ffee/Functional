...
def main():
    ...
    <exception cref="SystemExit">When build_zip.py has not produced the skin zip first.</exception>
    <remarks>
    Zips and verifies the repository add-on, copies the install once zip to
    Builds/, copies (never rebuilds) the already tested skin zip, clears
    superseded zips from both folders, then writes the combined addons.xml
    and its md5. The digest is written with no trailing newline because Kodi
    compares the fetched text byte for byte.
    </remarks>
    """
